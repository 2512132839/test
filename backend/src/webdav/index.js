...
export const webdavAuthMiddleware = async (c, next) => {
  ...
  // 如果没有认证头或不是Basic认证，返回401并请求认证
  if (!authHeader || !authHeader.toLowerCase().startsWith("basic ")) {
    console.log("WebDAV认证失败: 缺少Basic认证头");
    return createUnauthorizedResponse("Unauthorized: Missing or invalid Basic authentication");
  }
...
export async function handleWebDAV(c) {
  ...
  try {
    ...
    if (userType === "apiKey" && authInfo && authInfo.apiKey) {
      ...
      if (!hasMountPermission) {
        console.log(`WebDAV请求拒绝: ${method} ${path}, 挂载权限不足`);
