...
export class FileService {
  ...
  async getFileBySlug(slug) {
    ...
    const file = await this.fileRepository.findBySlugWithS3Config(slug);
  ...
  async incrementAndCheckFileViews(slug) {
    ...
    const updatedFile = await this.fileRepository.findBySlugWithS3Config(slug);
  ...
  async generateFileDownloadUrl(file, encryptionSecret, request = null) {
    ...
    if (file.s3_config_id && file.storage_path) {
      const s3Config = await this.s3ConfigRepository.findById(file.s3_config_id);
      if (s3Config) {
        try {
          // 生成预览URL，使用S3配置的默认时效
   ...
   */
  async deleteFileRecordByStoragePath(s3ConfigId, storagePath) {
    return await this.fileRepository.deleteByStoragePath(s3ConfigId, storagePath);
  }
  ...
  async getAdminFileList(options = {}) {
    const { limit = 30, offset = 0, createdBy, s3ConfigId } = options;

    // 构建查询条件
    const conditions = {};
    if (createdBy) conditions.created_by = createdBy;
    if (s3ConfigId) conditions.s3_config_id = s3ConfigId;

    // 获取文件列表
    const files = await this.fileRepository.findManyWithS3Config(conditions, {
  ...
  async getAdminFileDetail(fileId, encryptionSecret, request = null) {
    // 获取文件详情
    const file = await this.fileRepository.findByIdWithS3Config(fileId);
    if (!file) {
      throw new Error("文件不存在");
    }
...
export async function deleteFileRecordByStoragePath(db, s3ConfigId, storagePath) {
  const fileService = new FileService(db);
  return await fileService.deleteFileRecordByStoragePath(s3ConfigId, storagePath);
}
