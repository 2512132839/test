    ...
    async () => {
      ...
      if (useMultipart) {
        ...
        if (s3SubPath && s3SubPath.trim() !== "") {
          ...
          // 处理根路径的情况
          const rootPrefix = s3Config.root_prefix || "";
          const defaultFolder = s3Config.default_folder || "";

          // 确保路径组件之间只有一个斜杠
          let combinedPrefix = "";
          if (rootPrefix) {
            combinedPrefix = rootPrefix.endsWith("/") ? rootPrefix : rootPrefix + "/";
          }
          if (defaultFolder) {
            if (combinedPrefix && !combinedPrefix.endsWith("/")) {
              combinedPrefix += "/";
            }
            combinedPrefix += defaultFolder.endsWith("/") ? defaultFolder : defaultFolder + "/";
          }

          // 构建最终路径
          finalS3Path = combinedPrefix + fileName;

          console.log(`上传文件：在根路径使用配置的前缀 - rootPrefix=${rootPrefix}, defaultFolder=${defaultFolder}, finalPath=${finalS3Path}`);
        }
